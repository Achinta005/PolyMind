...
import {
  ...
  Layout, Target, X, Plus, Minus, LucideIcon, MessageSquare, Trash2,
} from "lucide-react";
...
// ─── Domain types ─────────────────────────────────────────────────────────────
interface Citation { docName: string; page: number; chunk: string }
interface Doc { id: string; filename: string; size_bytes: number; status: string; created_at: string }
interface Session { id: string; title: string; documentIds: string[]; updatedAt: string }
interface Message {
  id: string;
  role: "user" | "assistant";
  content: string;
...
// ─── Parsed block types ───────────────────────────────────────────────────────
type SectionKey = "summary" | "strength" | "weakness" | "content" | "format" | "relevance" | "score" | "default";
...
function parseContent(text: string): ParsedBlock[] | null {
  ...
  while (i < lines.length) {
    ...
    if (overallScore) { blocks.push({ type: "overall-score", value: parseFloat(overallScore[1]) }); i++; continue; }
    ...
    if (sectionHeader && !line.match(/^\d+\.\s/)) {
      ...
      while (i < lines.length) {
        ...
        if (scoreMatch) items.push({ type: "score-item", label: scoreMatch[1], weight: parseInt(scoreMatch[2]), value: parseInt(scoreMatch[3]) });
        else if (numbered) items.push({ type: "numbered", num: numbered[1], label: numbered[2], text: numbered[3] });
        else if (bullet) items.push({ type: "bullet", value: bullet[1] });
        else if (sub && !sub.match(/^\*\*[^*]+\*\*[:：]?\s*$/)) items.push({ type: "text", value: sub });
        else break;
        i++;
      }
      if (title || items.length > 0) blocks.push({ type: "section", title, items, icon: iconForTitle(title) });
      continue;
    }
...
interface SectionStyle { icon: LucideIcon; accent: string; bg: string; border: string }
...
// ─── Score Ring ───────────────────────────────────────────────────────────────
const ScoreRing: FC<{ value: number; max?: number; size?: number; accent?: string }> = ({ value, max = 10, size = 72, accent = "#c8f04d" }) => {
  const r = (size - 12) / 2;
  ...
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)", flexShrink: 0 }}>
      ...
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={accent} strokeWidth={7} strokeDasharray={`${filled} ${circ}`} strokeLinecap="round" />
      <text x={size/2} y={size/2} textAnchor="middle" dominantBaseline="central" fill="white" fontSize={size*0.23} fontWeight="800" fontFamily="monospace" style={{ transform: "rotate(90deg)", transformOrigin: `${size/2}px ${size/2}px` }}>{value}</text>
    </svg>
  );
};

// ─── Score Bar ────────────────────────────────────────────────────────────────
const ScoreBar: FC<{ label: string; weight: number; value: number; accent: string }> = ({ label, weight, value, accent }) => (
  <div style={{ marginBottom: 10 }}>
    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
      <span style={{ fontSize: 11, color: "rgba(255,255,255,0.55)", fontFamily: "monospace" }}>{label} <span style={{ color: "rgba(255,255,255,0.25)" }}>({weight}%)</span></span>
...
// ─── Plain / Structured Message ───────────────────────────────────────────────
const PlainMessage: FC<{ text: string }> = ({ text }) => (
  <p style={{ fontSize: 13, lineHeight: 1.75, color: "rgba(255,255,255,0.82)", margin: 0, whiteSpace: "pre-wrap" }}>{text}</p>
);
...
const StructuredMessage: FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {blocks.map((block, bi) => {
        if (block.type === "plain") return <p key={bi} style={{ fontSize: 13, lineHeight: 1.75, color: "rgba(255,255,255,0.78)", margin: 0 }}>{block.value}</p>;
        if (block.type === "overall-score") {
          const label = block.value >= 9 ? "Exceptional" : block.value >= 8 ? "Strong Candidate" : block.value >= 6.5 ? "Above Average" : block.value >= 5 ? "Moderate" : "Needs Work";
          return (
            <div key={bi} style={{ display: "flex", alignItems: "center", gap: 18, background: "rgba(200,240,77,0.06)", border: "1px solid rgba(200,240,77,0.2)", borderRadius: 14, padding: "14px 18px" }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 30, fontWeight: 900, color: "#c8f04d", lineHeight: 1, fontFamily: "monospace" }}>{block.value}<span style={{ fontSize: 14, color: "rgba(200,240,77,0.45)", fontWeight: 400 }}>/10</span></div>
          ...
          return (
            <div key={bi} style={{ background: s.bg, border: `1px solid ${s.border}`, borderRadius: 12, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "10px 14px", borderBottom: `1px solid ${s.border}`, background: "rgba(0,0,0,0.12)" }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: s.accent, fontFamily: "monospace", letterSpacing: "0.06em" }}>{block.title.toUpperCase()}</span>
              </div>
              <div style={{ padding: "12px 14px" }}>
                {scoreItems.length > 0 && <div style={{ marginBottom: otherItems.length > 0 ? 12 : 0 }}>{scoreItems.map((it, ii) => <ScoreBar key={ii} label={it.label} weight={it.weight} value={it.value} accent={s.accent} />)}</div>}
                {otherItems.map((item, ii) => {
                  if (item.type === "text") return <p key={ii} style={{ fontSize: 13, lineHeight: 1.7, color: "rgba(255,255,255,0.72)", margin: "0 0 6px" }}>{item.value}</p>;
...
// ─── Document Selector ────────────────────────────────────────────────────────
const DocSelector: FC<{ docs: Doc[]; loading: boolean; selectedIds: Set<string>; onToggle: (id: string) => void }> = ({ docs, loading, selectedIds, onToggle }) => {
  ...
  return (
    <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", background: "#06090f" }}>
      <button onClick={() => setOpen(v => !v)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 18px", background: "transparent", border: "none", cursor: "pointer", boxSizing: "border-box" }}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, flex: 1 }}>
          <div style={{ display: "flex", gap: 3, flexShrink: 0 }}>
            {Array.from({ length: 5 }).map((_, i) => {
              ...
              return (
                <div key={i} title={doc?.filename} style={{ width: 22, height: 22, borderRadius: 6, background: doc ? "rgba(200,240,77,0.12)" : "rgba(255,255,255,0.04)", border: doc ? "1px solid rgba(200,240,77,0.32)" : "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {doc ? <FileText size={10} color="#c8f04d" /> : <span style={{ color: "rgba(255,255,255,0.12)", fontSize: 9, fontFamily: "monospace" }}>{i + 1}</span>}
                </div>
              );
            })}
          </div>
          {selected.length === 0
            ? <span style={{ fontSize: 11, color: "rgba(255,255,255,0.28)", fontFamily: "monospace" }}>Select documents to search — required</span>
            : <div style={{ display: "flex", gap: 5, flexWrap: "wrap", minWidth: 0 }}>
                {selected.map(d => <span key={d.id} style={{ fontSize: 10, fontFamily: "monospace", color: "rgba(200,240,77,0.75)", background: "rgba(200,240,77,0.07)", border: "1px solid rgba(200,240,77,0.18)", borderRadius: 5, padding: "1px 7px", maxWidth: 130, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.filename.replace(/\.pdf$/i, "")}</span>)}
              </div>
          }
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, marginLeft: 10 }}>
          {selected.length > 0 && <span style={{ fontSize: 10, fontFamily: "monospace", color: "rgba(200,240,77,0.55)", background: "rgba(200,240,77,0.08)", border: "1px solid rgba(200,240,77,0.15)", borderRadius: 100, padding: "1px 8px" }}>{selected.length}/5</span>}
      ...
      {open && (
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", background: "#080d17", padding: "14px 16px", maxHeight: 280, overflowY: "auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 9, padding: "7px 11px", marginBottom: 10 }}>
            ...
            {search && <button onClick={() => setSearch("")} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}><X size={11} color="rgba(255,255,255,0.3)" /></button>}
          </div>
          {loading ? (
            <div style={{ textAlign: "center", padding: "20px 0", color: "rgba(255,255,255,0.2)", fontSize: 12, fontFamily: "monospace" }}>Loading…</div>
          ) : filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "20px 0", color: "rgba(255,255,255,0.15)", fontSize: 12, fontFamily: "monospace" }}>{ready.length === 0 ? "No documents uploaded yet" : "No matches found"}</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
              {filtered.map(doc => {
                ...
                return (
                  <button key={doc.id} onClick={() => !maxed && onToggle(doc.id)} disabled={maxed}
                    ...
                    <div style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, background: sel ? "#c8f04d" : "transparent", border: sel ? "none" : "1.5px solid rgba(255,255,255,0.18)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {sel && <Check size={10} color="#050810" strokeWidth={3} />}
                    </div>
                    <div style={{ width: 30, height: 30, borderRadius: 8, flexShrink: 0, background: sel ? "rgba(200,240,77,0.1)" : "rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <FileText size={13} color={sel ? "#c8f04d" : "rgba(255,255,255,0.28)"} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: sel ? "rgba(255,255,255,0.92)" : "rgba(255,255,255,0.55)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{doc.filename}</div>
                      <div style={{ fontSize: 10, fontFamily: "monospace", color: "rgba(255,255,255,0.2)", marginTop: 2 }}>{formatBytes(doc.size_bytes)} · {formatDate(doc.created_at)}</div>
                    </div>
          ...
          {selectedIds.size >= 5 && <p style={{ margin: "10px 0 0", textAlign: "center", fontSize: 10, fontFamily: "monospace", color: "rgba(200,240,77,0.4)" }}>Maximum 5 documents selected</p>}
        </div>
      )}
    </div>
  );
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setDocsLoading(true);
        const res = await fetch(`${API}/polymind/documents`, { headers: { Authorization: `Bearer ${accessToken}` }, credentials: "include" });
  ...
  useEffect(() => {
    if (!accessToken) return;
    fetch(`${API}/polymind/history/sessions`, { headers: { Authorization: `Bearer ${accessToken}` }, credentials: "include" })
      .then(r => r.json())
  ...
  const loadSession = async (session: Session) => {
    ...
    setMessages([]);
    try {
      const res = await fetch(`${API}/polymind/history/sessions/${session.id}/messages`, {
  ...
  const deleteSession = async (sessionId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    await fetch(`${API}/polymind/history/sessions/${sessionId}`, {
  ...
  const sendMessage = async () => {
    ...
    const userMsgId = uid();
    setMessages(prev => [...prev, { id: userMsgId, role: "user", content, selectedDocs: [...selectedDocs] }]);
    setLoading(true);

    const assistantId = uid();
    setMessages(prev => [...prev, { id: assistantId, role: "assistant", content: "", citations: [], isStreaming: true }]);

    try {
      const res = await fetch(`${API}/polymind/documents/query`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}`, Accept: "text/event-stream" },
        body: JSON.stringify({ question: content, document_ids: [...selectedDocIds], session_id: currentSessionId ?? undefined }),
      });

      if (!res.ok && !res.headers.get("content-type")?.includes("text/event-stream")) {
        const err = await res.json().catch(() => ({ detail: "Unknown error" }));
        throw new Error((err as any).detail ?? (err as any).message ?? "Request failed");
      }
      ...
      while (true) {
        ...
        for (const line of decoder.decode(value).split("\n")) {
          ...
          try {
            const data = JSON.parse(line.slice(6)) as { token?: string; citations?: Citation[]; sessionId?: string; error?: string; done?: boolean };
            if (data.error) throw new Error(data.error);
            if (data.token) fullText += data.token;
            if (data.citations) citations = data.citations;

            // ── capture new sessionId from backend ──
            if (data.sessionId && !currentSessionId) {
              setCurrentSessionId(data.sessionId);
              setSessions(prev => {
                const exists = prev.find(s => s.id === data.sessionId);
                if (exists) return prev;
                return [{ id: data.sessionId!, title: content.slice(0, 40), documentIds: [...selectedDocIds], updatedAt: new Date().toISOString() }, ...prev];
              });
            }

            setMessages(prev => prev.map(m => m.id === assistantId ? { ...m, content: fullText, citations, isStreaming: !data.done } : m));
          } catch (e) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "#050810", color: "white", fontFamily: "'DM Sans', sans-serif", position: "relative" }}>
      <style>{`
        @keyframes bounce { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-4px)} }
        ::-webkit-scrollbar{width:4px} ::-webkit-scrollbar-track{background:transparent}
        ::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.07);border-radius:2px}
        textarea::placeholder{color:rgba(255,255,255,0.18)} input::placeholder{color:rgba(255,255,255,0.2)}
      `}</style>

      {/* ── Session Sidebar ── */}
      {sidebarOpen && (
        <div style={{ position: "absolute", top: 0, left: 0, width: 260, height: "100%", background: "#080d17", borderRight: "1px solid rgba(255,255,255,0.06)", zIndex: 10, display: "flex", flexDirection: "column" }}>
          <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 12, fontWeight: 700 }}>Chat History</span>
            <button onClick={() => setSidebarOpen(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.3)", fontSize: 16 }}>✕</button>
          </div>
          <div style={{ padding: "8px" }}>
            <button onClick={newChat} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, background: "rgba(200,240,77,0.07)", border: "1px solid rgba(200,240,77,0.18)", color: "#c8f04d", fontSize: 12, fontFamily: "monospace", cursor: "pointer", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <Plus size={11} /> New Chat
            </button>
          </div>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 8px 8px" }}>
            {sessions.length === 0
              ? <p style={{ textAlign: "center", color: "rgba(255,255,255,0.2)", fontSize: 12, fontFamily: "monospace", padding: "20px 0" }}>No sessions yet</p>
              : sessions.map(s => (
                <div key={s.id} onClick={() => loadSession(s)}
                  style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 10px", borderRadius: 8, cursor: "pointer", background: currentSessionId === s.id ? "rgba(200,240,77,0.07)" : "transparent", border: currentSessionId === s.id ? "1px solid rgba(200,240,77,0.18)" : "1px solid transparent", marginBottom: 3 }}>
                  <MessageSquare size={12} color="rgba(200,240,77,0.5)" style={{ flexShrink: 0 }} />
                  <span style={{ flex: 1, fontSize: 12, color: "rgba(255,255,255,0.65)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.title}</span>
                  <button onClick={e => deleteSession(s.id, e)} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", flexShrink: 0 }}>
                    <Trash2 size={11} />
                  </button>
                </div>
              ))
      ...
      <div style={{ borderBottom: "1px solid rgba(255,255,255,0.05)", padding: "13px 22px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        <button onClick={() => setSidebarOpen(v => !v)}
          style={{ width: 34, height: 34, background: sidebarOpen ? "rgba(200,240,77,0.15)" : "rgba(200,240,77,0.08)", border: "1px solid rgba(200,240,77,0.14)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <Bot size={16} color="#c8f04d" />
        </button>
        <div style={{ flex: 1 }}>
          <h1 style={{ margin: 0, fontSize: 15, fontWeight: 800, letterSpacing: "-0.02em" }}>Research Chat</h1>
          <p style={{ margin: 0, fontSize: 10, color: "rgba(255,255,255,0.26)", fontFamily: "monospace" }}>
            {currentSessionId ? "Session active" : "Ask anything about your documents"}
          </p>
        </div>
        <button onClick={newChat}
          style={{ fontSize: 11, fontFamily: "monospace", color: "rgba(200,240,77,0.6)", background: "rgba(200,240,77,0.07)", border: "1px solid rgba(200,240,77,0.18)", borderRadius: 6, padding: "5px 12px", cursor: "pointer" }}>
          + New Chat
        </button>
      </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
              {["Summarize the main findings", "What methodology was used?", "Compare the two papers"].map(q => (
                <button key={q} onClick={() => setInput(q)}
                  style={{ border: "1px solid rgba(255,255,255,0.09)", color: "rgba(255,255,255,0.4)", fontSize: 12, padding: "6px 14px", borderRadius: 100, background: "transparent", cursor: "pointer", fontFamily: "inherit" }}
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "flex-end" }}>
                  {msg.selectedDocs.map(d => (
                    <span key={d.id} style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(200,240,77,0.06)", border: "1px solid rgba(200,240,77,0.16)", borderRadius: 5, padding: "2px 7px", fontSize: 10, fontFamily: "monospace", color: "rgba(200,240,77,0.6)", maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      <FileText size={8} style={{ flexShrink: 0 }} />{d.filename.replace(/\.pdf$/i, "")}
                    </span>
                  ))}
                </div>
              )}
              <div style={{ borderRadius: msg.role === "user" ? "16px 4px 16px 16px" : "4px 16px 16px 16px", padding: "12px 15px", background: msg.role === "user" ? "rgba(200,240,77,0.07)" : "rgba(255,255,255,0.04)", border: msg.role === "user" ? "1px solid rgba(200,240,77,0.16)" : "1px solid rgba(255,255,255,0.07)" }}>
                {msg.role === "assistant" && msg.content
                  ...
                  : msg.isStreaming && !msg.content
                    ? <span style={{ display: "flex", gap: 5, alignItems: "center", padding: "4px 0" }}>{[0,1,2].map(i => <span key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "rgba(255,255,255,0.28)", animation: "bounce 1s infinite", animationDelay: `${i*0.15}s`, display: "inline-block" }} />)}</span>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                  {msg.citations.map((cite, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(200,240,77,0.05)", border: "1px solid rgba(200,240,77,0.13)", borderRadius: 6, padding: "3px 8px" }}>
                      ...
                      <span style={{ fontSize: 10, fontFamily: "monospace", color: "rgba(200,240,77,0.65)" }}>{cite.docName} · p.{cite.page}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "11px 16px", flexShrink: 0 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "flex-end", maxWidth: 860, margin: "0 auto" }}>
          <div style={{ flex: 1, background: "#0c1120", border: selectedDocIds.size > 0 ? "1px solid rgba(200,240,77,0.18)" : "1px solid rgba(255,255,255,0.07)", borderRadius: 13, overflow: "hidden", transition: "border-color 0.2s", boxShadow: selectedDocIds.size > 0 ? "0 0 0 3px rgba(200,240,77,0.04)" : "none" }}>
            <textarea ref={textareaRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown}
              ...
              rows={1} style={{ width: "100%", background: "transparent", padding: "11px 15px", fontSize: 13, color: "white", resize: "none", border: "none", outline: "none", fontFamily: "monospace", maxHeight: 120, lineHeight: 1.6, boxSizing: "border-box", caretColor: "#c8f04d" }} />
          </div>
          <button onClick={sendMessage} disabled={!input.trim() || loading}
            style={{ width: 38, height: 38, borderRadius: 11, border: "none", cursor: "pointer", background: input.trim() && selectedDocIds.size > 0 ? "#c8f04d" : "rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, opacity: !input.trim() || loading ? 0.38 : 1 }}
        ...
        <p style={{ textAlign: "center", fontSize: 10, color: "rgba(255,255,255,0.13)", fontFamily: "monospace", margin: "7px 0 0" }}>Enter to send · Shift+Enter for new line</p>
      </div>
    </div>
  );
}