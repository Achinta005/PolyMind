...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    ...
    const fetchDocuments = async () => {
      setLoadingDocs(true);
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/polymind/documents`, {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/polymind/documents/${docId}/status`,
          { headers: { Authorization: `Bearer ${accessToken}` } }
        );
        const data = await res.json();
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/polymind/documents/ingest`, {
        method: "POST",
        headers: { Authorization: `Bearer ${accessToken}` },
        body: form,
      });
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/polymind/documents/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${accessToken}` },
      });
